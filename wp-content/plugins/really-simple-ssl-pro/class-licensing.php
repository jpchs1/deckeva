...
if (!class_exists("rsssl_licensing")) {
	class rsssl_licensing {
		...
		function __construct() {
			...
			add_filter( 'rsssl_do_action', array( $this, 'rest_api_license_activate' ), 10, 3 );
		 ...
		 * @return array
		 */
		public function rest_api_license_activate($data, $test, $request) {
			if (!rsssl_user_can_manage()) {
				return $data;
			}

			if ( $test==='activate_license' ) {
				...
				$this->get_license_status('activate_license', true );
				return $this->license_notices($data, $test, $request);
			}
		 ...
		 * @return array
		 */
		public function rest_api_license( array $data, string $test, WP_REST_Request $request): array {
			if (!rsssl_user_can_manage()) {
				return $data;
			}
			switch ($test) {
		...
		public function license_notices($data, $test, $request){
			if (!rsssl_user_can_manage()) {
				return $data;
			}
			$status = $this->get_license_status();
