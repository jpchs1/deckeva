...
* The Bulgarian Lev currency is being deprecated in favor of the Euro on 01.01.2026
* This version handles this currency transition for Stripe
* All recurring payments generated by the plugin starting with 29.12.2025 will be automatically converted from BGN to EUR, at the government agreed exchange rate of 1 EUR = 1.95583 BGN
* For new payments to work correctly, if your default currency is BGN, **you are required** to manually switch to EUR by going to Settings -> Payments and also modify your Subscription Plan prices accordingly

**Enhancements:**

* Added an option when editing Member Subscriptions as an admin that can be checked in order to not send any emails in relation to the changes you are doing
* Allow full subscription management by the admin through the back-end interface with the use of a filter: pms_edit_subscription_enable_billing_next_payment_editing

**Fixes:**

* Security issue related to Insecure Direct Object Reference when cancelling subscriptions. Thanks to daroo and the Patchstack team
* Issue with PayPal button appearing when Manual / Offline payment gateway was selected by default
* The automatically inserted link from the GDPR field is now opening in a new tab
* Disable the ability for admins to modify the Expiration Date field for PayPal recurring subscriptions
* Fix a notice that could be triggered in some cases from the WooCommerce Integration
* A notice that was triggered when the Stripe webhook URL was accesed manually
* An error that could be triggered from reCaptcha in some situations
* Error being thrown from TutorLMS integration when an admin was manually adjusting the subscription plan from the back-end
* Properly show Billing Fields errors on the Profile Builder form when an AJAX checkout is performed
